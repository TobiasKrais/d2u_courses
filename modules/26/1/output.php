...
if (rex::isBackend()) {
    ...
    elseif ($category instanceof Category) {
        if (count($category->getChildren(true)) > 0) {
            ...
            foreach ($category->getChildren(true) as $child_category) {
                printBoxModule26_1($child_category->name, $child_category->picture, $child_category->color, $child_category->color_dark, $child_category->getUrl(), $box_per_line);
            }
        } else {
    ...
    elseif (\TobiasKrais\D2UCourses\Extension::isActive('locations') && $location_category instanceof LocationCategory) {
    ...
    // Deal with locations
    elseif (\TobiasKrais\D2UCourses\Extension::isActive('locations') && $location instanceof Location) {
    ...
    elseif ($schedule_category instanceof ScheduleCategory) {
        if (count($schedule_category->getChildren(true)) > 0) {
            ...
            foreach ($schedule_category->getChildren(true) as $child_schedule_category) {
                printBoxModule26_1($child_schedule_category->name, $child_schedule_category->picture, (string) rex_config::get('d2u_courses', 'schedule_category_bg_color', '#66ccc2'), (string) rex_config::get('d2u_courses', 'dark_schedule_category_bg_color', (string) rex_config::get('d2u_courses', 'schedule_category_bg_color', '#66ccc2')), $child_schedule_category->getUrl(), $box_per_line);
            }
        } else {
    ...
    elseif ($target_group instanceof TargetGroup) {
        if (count($target_group->getChildren()) > 0) {
            ...
            foreach ($target_group->getChildren() as $child_target_group) {
                printBoxModule26_1($child_target_group->name, $child_target_group->picture, (string) rex_config::get('d2u_courses', 'target_group_bg_color', '#b57a00'), (string) rex_config::get('d2u_courses', 'dark_target_group_bg_color', (string) rex_config::get('d2u_courses', 'target_group_bg_color', '#b57a00')), $child_target_group->getUrl(), $box_per_line);
            }
        } else {
    ...
    // Nothing requested: selected startpage
        ...
        if ('locations' === $startpage && \TobiasKrais\D2UCourses\Extension::isActive('locations')) { /** @phpstan-ignore-line */
            ...
            foreach ($location_categories as $current_location_category) {
                printBoxModule26_1($current_location_category->name, $current_location_category->picture, (string) rex_config::get('d2u_courses', 'location_bg_color', '#41b23b'), (string) rex_config::get('d2u_courses', 'dark_location_bg_color', (string) rex_config::get('d2u_courses', 'location_bg_color', '#41b23b')), $current_location_category->getUrl(), $tmp_box_per_line);
            }
        } elseif ('schedule_categories' === $startpage && \TobiasKrais\D2UCourses\Extension::isActive('schedule_categories')) { /** @phpstan-ignore-line */
            $schedule_categories = ScheduleCategory::getAllParents(true);
            foreach ($schedule_categories as $currecnt_schedule_category) {
                printBoxModule26_1($currecnt_schedule_category->name, $currecnt_schedule_category->picture, (string) rex_config::get('d2u_courses', 'schedule_category_bg_color', '#66ccc2'), (string) rex_config::get('d2u_courses', 'dark_schedule_category_bg_color', (string) rex_config::get('d2u_courses', 'schedule_category_bg_color', '#66ccc2')), $currecnt_schedule_category->getUrl(), $tmp_box_per_line);
            }
        } elseif ('target_groups' === $startpage && \TobiasKrais\D2UCourses\Extension::isActive('target_groups')) { /** @phpstan-ignore-line */
            $target_groups = TargetGroup::getAll(true);
            foreach ($target_groups as $current_target_group) {
                printBoxModule26_1($current_target_group->name, $current_target_group->picture, (string) rex_config::get('d2u_courses', 'target_group_bg_color', '#b57a00'), (string) rex_config::get('d2u_courses', 'dark_target_group_bg_color', (string) rex_config::get('d2u_courses', 'target_group_bg_color', '#b57a00')), $current_target_group->getUrl(), $tmp_box_per_line);
            }
        } elseif (false === $course) {
            ...
            if (count($categories) === 0) {
                ...
                foreach ($categories as $current_category) {
                    printBoxModule26_1($current_category->name, $current_category->picture, $current_category->color, $current_category->color_dark, $current_category->getUrl(), $tmp_box_per_line);
                }
            }
    ...
    // Course list
    if ('active' === (string) rex_config::get('d2u_courses', 'forward_single_course', 'inactive') && 1 === count($courses) && null === filter_input(INPUT_POST, 'course_search')) {
        ...
        echo '<div class="col-12">';
        echo '<div class="search_title">';
        echo '<h1>'. \Sprog\Wildcard::get('d2u_courses_search_no_hits') .'</h1>';
        echo '<p>'. \Sprog\Wildcard::get('d2u_courses_search_no_hits_text') .'</p>';
        echo '</div>';
        echo '</div>';
    }
    ...
    if ($course instanceof Course) {
        ...
        echo '<div class="course_title"'. ($course->category instanceof Category ? ' style="'. TobiasKrais\D2UCourses\FrontendHelper::getThemeColorStyle('background-color', $course->category->color, $course->category->color_dark, '#5e5c64') .'"' : '') .'>';
        echo '<h1>'. rex_escape($course->name);
        ...
        if ('' !== $course->instructor) {
            ...
            $box_details .= '<div class="course_box spacer_box"><b>'. \Sprog\Wildcard::get('d2u_courses_instructor') .':</b> '. $course->instructor .'</div>';
            $box_details .= '</div>';
        }

        if ('' !== $course->date_start || '' !== $course->date_end || '' !== $course->time) {
            $box_details .= '<div class="col-12 course_row">';
            $box_details .= '<div class="course_box spacer_box"><b>'. \Sprog\Wildcard::get('d2u_courses_date') .':</b> ';
            if ('' !== $course->date_start || '' !== $course->date_end || '' !== $course->time) {
                $date = '';
    ...
    //					$date .= ' '. \Sprog\Wildcard::get('d2u_courses_oclock');
    //				}
                }
                $box_details .= $date .'<br>';
            }
        ...
        if ('' !== trim($course->details_deadline)) {
            ...
            $box_details .= '<b>'. \Sprog\Wildcard::get('d2u_courses_registration_deadline') .':</b> '. $course->details_deadline;
            $box_details .= '</div>';
            $box_details .= '</div>';
        }
        ...
        if ($course->price > 0 || ($course->price_salery_level && count($course->price_salery_level_details) > 0)) {
            $box_details .= '<div class="col-12 course_row">';
            $box_details .= '<div class="course_box spacer_box"><b>'. \Sprog\Wildcard::get('d2u_courses_fee') .':</b> ';
            if ($course->price_salery_level) {
                $box_details .= \Sprog\Wildcard::get('d2u_courses_price_salery_level_details') .'<br>';
                $box_details .= '<select class="participant" name="participant_price_salery_level_row_add_selector"'. ($course->category instanceof Category ? ' style="'. TobiasKrais\D2UCourses\FrontendHelper::getThemeColorStyle('border-color', $course->category->color, $course->category->color_dark, '#5e5c64') .'"' : '') .'>';
                ...
                if ($course->price_discount > 0 && $course->price_discount < $course->price) {
                    $box_details .= ' ('. \Sprog\Wildcard::get('d2u_courses_discount') .': '. number_format($course->price_discount, 2, ',', '.') .' €)';
                }
        ...
        if ('' !== trim($course->details_course)) {
            $box_details .= '<div class="col-12 course_row">';
            $box_details .= '<div class="course_box spacer_box">';
            $box_details .= '<b>'. \Sprog\Wildcard::get('d2u_courses_details_course') .':</b> '. $course->details_course;
            $box_details .= '</div>';
            $box_details .= '</div>';
        }
        ...
        if ($course->participants_max > 0) {
            $box_details .= '<div class="col-12 course_row">';
            $box_details .= '<div class="course_box spacer_box">';
            $box_details .= '<b>'. \Sprog\Wildcard::get('d2u_courses_participants') .': '. $course->participants_max .'</b> ('. \Sprog\Wildcard::get('d2u_courses_max') .')';
            if ($course->participants_min > 0) {
                $box_details .= ' / <b>'. $course->participants_min .'</b> ('. \Sprog\Wildcard::get('d2u_courses_min') .')';
            }
            if ($course->participants_number > 0) {
                $box_details .= ' / <b>'. $course->participants_number .'</b> ('. \Sprog\Wildcard::get('d2u_courses_booked') .')';
            }
            if ($course->participants_wait_list > 0) {
                $box_details .= ' / <b>'. $course->participants_wait_list .'</b> ('. \Sprog\Wildcard::get('d2u_courses_wait_list') .')';
            }
            $box_details .= '<br></div>';
            $box_details .= '</div>';
        }
        ...
        if ($course->redaxo_article > 0 || '' !== $course->url_external || count($course->downloads) > 0) {
            if ($course->redaxo_article > 0 || '' !== $course->url_external) {
                $box_details .= '<div class="col-12 course_row">';
                $box_details .= '<div class="course_box spacer_box"><b>'. \Sprog\Wildcard::get('d2u_courses_infolink') .':</b> ';
            ...
            if (count($course->downloads) > 0) {
                $box_details .= '<div class="col-12 course_row">';
                $box_details .= '<div class="course_box spacer_box"><b>'. \Sprog\Wildcard::get('d2u_courses_downloads') .':</b> ';
                $box_details .= '<ul>';
        ...
        if (\TobiasKrais\D2UCourses\Extension::isActive('locations')) {
            ...
            $box_details .= '<div class="course_box spacer_box">';
            if ('' !== $course->room) {
                $box_details .= '<b>'. \Sprog\Wildcard::get('d2u_courses_locations_room') .':</b><br />';
                $box_details .= $course->room;
                if ($course->location instanceof Location && '' !== $course->location->site_plan) {
                    $box_details .= ' (<a href="'. rex_url::media($course->location->site_plan) .'" target="_blank">'. \Sprog\Wildcard::get('d2u_courses_locations_site_plan') .'</a>)';
                }
                $box_details .= '<br><br>';
            } else {
                if ($course->location instanceof Location && '' !== $course->location->site_plan) {
                    $box_details .= '<b><a href="'. rex_url::media($course->location->site_plan) .'" target="_blank">'. \Sprog\Wildcard::get('d2u_courses_locations_site_plan') .'</a></b><br><br>';
                }
            }
            if ($course->location instanceof Location) {
                $box_details .= '<b>'. \Sprog\Wildcard::get('d2u_courses_locations_city') .':</b><br />';
        ...
        if ('yes' === $course->registration_possible || 'yes_number' === $course->registration_possible || 'booked' === $course->registration_possible) {
            $box_details .= '<div class="col-12 course_row">';
            $box_details .= '<div class="course_box spacer_box add_cart"'. ($course->category instanceof Category ? ' style="'. TobiasKrais\D2UCourses\FrontendHelper::getThemeColorStyle('background-color', $course->category->color, $course->category->color_dark, '#5e5c64') .'"' : '') .'>';
            if (TobiasKrais\D2UCourses\Cart::getCart()->hasCourse($course->course_id)) {
                $box_details .= '<a href="'. rex_getUrl((int) rex_config::get('d2u_courses', 'article_id_shopping_cart', rex_article::getSiteStartArticleId())) .'">'. \Sprog\Wildcard::get('d2u_courses_cart_course_already_in') .' - '. \Sprog\Wildcard::get('d2u_courses_cart_go_to') .'</a>';
            } else {
                ...
                $box_details .= '<input type="submit" class="add_cart" name="submit" value="'. \Sprog\Wildcard::get('d2u_courses_cart_add') .'"'. ($course->category instanceof Category ? ' style="'. TobiasKrais\D2UCourses\FrontendHelper::getThemeColorStyle('background-color', $course->category->color, $course->category->color_dark, '#5e5c64') .'"' : '') .'>';
            }
            $box_details .= '</div>';
            $box_details .= '</div>';
