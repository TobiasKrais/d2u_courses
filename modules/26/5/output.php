...
if (isset($form_data['invoice_form'])) {
    ...
    if (filter_input(INPUT_POST, 'e-mail') !== false && '' !== filter_input(INPUT_POST, 'e-mail')) {
        ...
        echo '<h1>'. \Sprog\Wildcard::get('d2u_courses_cart_error') .'</h1>'
            .'<p>'. \Sprog\Wildcard::get('d2u_courses_cart_error_honeypot') .'</p>';
        echo '</div>';        
    }
    else {
...
//					    }
    ...
    //    }
        ...
        if (false === $error) {
            echo '<div class="col-12">';
            echo '<h1>'. \Sprog\Wildcard::get('d2u_courses_cart_thanks') .'</h1><p>'. \Sprog\Wildcard::get('d2u_courses_cart_thanks_details') .'</p>';
            echo '</div>';
            $cart->unsetCart();
        } else {
            echo '<div class="col-12">';
            echo '<h1>'. \Sprog\Wildcard::get('d2u_courses_cart_error') .'</h1>'
                .'<p>'. \Sprog\Wildcard::get('d2u_courses_cart_error_details') .'</p>';
            echo '</div>';
        }
    }
    ...
    if (count($payment_options) > 0) {
        echo \Sprog\Wildcard::get('d2u_courses_cart_bill_address');
    } else {
        echo \Sprog\Wildcard::get('d2u_courses_cart_contact_address');
    }
    echo '</h1></div>';
    ...
    if ((bool) rex_config::get('d2u_courses', 'allow_company', false)) {
        ...
        echo '<option value="P">'. \Sprog\Wildcard::get('d2u_courses_type_private') .'</option>';
        echo '<option value="B">'. \Sprog\Wildcard::get('d2u_courses_business') .'</option>';
        echo '</select>';
        echo '</p>';

        echo '<p>';
        echo '<label class="cart_text" for="invoice_form-company">'. \Sprog\Wildcard::get('d2u_courses_company') .' *</label>';
        echo '<input type="text" class="cart_text" name="invoice_form[company]" id="invoice_form-company" value="" maxlength="50">';
        echo '</p>';
    }
    echo '<p>';
    echo '<label class="cart_select" for="invoice_form-gender">'. \Sprog\Wildcard::get('d2u_courses_title') .'</label>';
    echo '<select class="cart_select" id="invoice_form-gender" name="invoice_form[gender]" size="1">';
    echo '<option value="W">'. \Sprog\Wildcard::get('d2u_courses_title_female') .'</option>';
    echo '<option value="M">'. \Sprog\Wildcard::get('d2u_courses_title_male') .'</option>';
    echo '</select>';
    echo '</p>';

    echo '<p>';
    echo '<label class="cart_text" for="invoice_form-firstname">'. \Sprog\Wildcard::get('d2u_courses_firstname') .' *</label>';
    echo '<input type="text" class="cart_text" name="invoice_form[firstname]" id="invoice_form-firstname" value="" maxlength="35" required>';
    echo '</p>';

    echo '<p>';
    echo '<label class="cart_text" for="invoice_form-lastname">'. \Sprog\Wildcard::get('d2u_courses_lastname') .' *</label>';
    echo '<input type="text" class="cart_text" name="invoice_form[lastname]" id="invoice_form-lastname" value="" maxlength="35" required>';
    echo '</p>';

    echo '<p>';
    echo '<label class="cart_text" for="invoice_form-address">'. \Sprog\Wildcard::get('d2u_courses_street') .' *</label>';
    echo '<input type="text" class="cart_text" name="invoice_form[address]" id="invoice_form-address" value="" maxlength="35" required>';
    echo '</p>';

    echo '<p>';
    echo '<label class="cart_text" for="invoice_form-zipcode">'. \Sprog\Wildcard::get('d2u_courses_zipcode') .' *</label>';
    echo '<input type="text" class="cart_text" name="invoice_form[zipcode]" id="invoice_form-zipcode" value="" maxlength="5" required>';
    echo '</p>';

    echo '<p>';
    echo '<label class="cart_text" for="invoice_form-city">'. \Sprog\Wildcard::get('d2u_courses_city') .' *</label>';
    echo '<input type="text" class="cart_text" name="invoice_form[city]" id="invoice_form-city" value="" maxlength="44" required>';
    echo '</p>';

    echo '<p>';
    echo '<label class="cart_text" for="invoice_form-country">'. \Sprog\Wildcard::get('d2u_courses_country') .' *</label>';
    ...
    echo '<p>';
    echo '<label class="cart_text" for="invoice_form-phone">'. \Sprog\Wildcard::get('d2u_courses_phone') .' *</label>';
    echo '<input type="text" class="cart_text" name="invoice_form[phone]" id="invoice_form-phone" value="" required>';
    echo '</p>';

    echo '<p>';
    echo '<label class="cart_text" for="invoice_form-e-mail">'. \Sprog\Wildcard::get('d2u_courses_email') .' *</label>';
...
<?php
    if ('active' === rex_config::get('d2u_courses', 'ask_vacation_pass', 'inactive') && false === \TobiasKrais\D2UCourses\Extension::isActive('kufer_sync')) {
        echo '<p>';
        echo '<label class="cart_text" for="invoice_form-vacation_pass">'. \Sprog\Wildcard::get('d2u_courses_vacation_pass') .'</label>';
    ...
    if (\TobiasKrais\D2UCourses\Extension::isActive('kufer_sync') && 'REX_VALUE[2]' === 'true') { /** @phpstan-ignore-line */
        echo '<div class="registration_header cart_row_title">'. \Sprog\Wildcard::get('d2u_courses_statistic') .'</div>';

        echo '<p>';
        echo '<label class="cart_text" for="invoice_form-birthday">'. \Sprog\Wildcard::get('d2u_courses_birthdate') .'</label>';
        echo '<input type="date" class="cart_date" name="invoice_form[birthday]" placeholder="'. \Sprog\Wildcard::get('d2u_courses_date_placeholder') .'" min="1900-01-01" max="'. (date('Y-m-d')) .'">';
        echo '</p>';
    }

    if (count($payment_options) > 0) {
        echo '<div class="registration_header cart_row_title"><h1>'. \Sprog\Wildcard::get('d2u_courses_payment_data') .'</h1></div>';

        echo '<p>';
        echo '<label class="cart_select" for="invoice_form-payment">'. \Sprog\Wildcard::get('d2u_courses_payment') .':</label>';
        ...
        if (in_array('direct_debit', $payment_options, true)) {
            echo '<option value="L">'. \Sprog\Wildcard::get('d2u_courses_payment_debit') .'</option>';
        }
        if (in_array('bank_transfer', $payment_options, true)) {
            echo '<option value="Ü">'. \Sprog\Wildcard::get('d2u_courses_payment_transfer') .'</option>';
        }
        if (in_array('cash', $payment_options, true)) {
            echo '<option value="B">'. \Sprog\Wildcard::get('d2u_courses_payment_cash') .'</option>';
        }
        echo '</select>';
        echo '</p>';

        echo '<p>';
        echo '<label class="cart_text" for="invoice_form-account_owner">'. \Sprog\Wildcard::get('d2u_courses_payment_account_owner') .' *</label>';
        ...
        echo '<p>';
        echo '<label class="cart_text" for="invoice_form-bank">'. \Sprog\Wildcard::get('d2u_courses_payment_bank') .' *</label>';
...
<?php
        ...
        echo '<p>';
        echo '<label class="cart_text" for="invoice_form-bic">'. \Sprog\Wildcard::get('d2u_courses_payment_bic') .' *</label>';
...
<script>
    ...
    function business_changer() {
        if($('#invoice_form-type').val() === 'B') {
            ...
            option.text = '<?= \Sprog\Wildcard::get('d2u_courses_payment_transfer') ?>';
            option.value = 'Ü';
...
<?php
    ...
    foreach (TobiasKrais\D2UCourses\Cart::getCourseIDs() as $course_id) {
        ...
        if ('' !== $course->date_start || '' !== $course->date_end || '' !== $course->time) {
            echo \Sprog\Wildcard::get('d2u_courses_date') .': ';
            $date = '';
        ...
        if (!$course->price_salery_level && $course->price > 0) {
            echo \Sprog\Wildcard::get('d2u_courses_fee') . ': '. number_format($course->price, 2, ',', '.') .' €';
            if ($course->price_discount > 0 && $course->price_discount < $course->price) {
                echo ' ('. \Sprog\Wildcard::get('d2u_courses_discount') .': '. number_format($course->price_discount, 2, ',', '.') .' €)<br>';
            }
        ...
        if ('yes_number' === $course->registration_possible) {
            ...
            if (array_key_exists('participant_number', $participant_data) && !is_array($participant_data['participant_number'])) {
                echo '<ul>';
                echo '<li>'. \Sprog\Wildcard::get('d2u_courses_participant_number') .': '. $participant_data['participant_number'];
                if ($course->price_salery_level) {
                    ...
                    foreach ($course->price_salery_level_details as $description => $price) {
                        ...
                        if ($counter_row_price_salery_level_details === (int) $participant_data['participant_price_salery_level_row_number'] && !is_array($participant_data['participant_price'])) {
                            echo ', '. \Sprog\Wildcard::get('d2u_courses_fee') .': '. $participant_data['participant_price'] .' ('. \Sprog\Wildcard::get('d2u_courses_price_salery_level') .': '. $description .')';
                            break;
                        }
                    }
                }
                ...
                if (is_array($participant)) {
                    ...
                    if ($ask_gender || ($course->category instanceof Category && in_array($course->category->getPartentRoot()->category_id, $ask_age_root_category_ids, true))) { /** @phpstan-ignore-line */
                        ...
                        if (1 === $ask_age && array_key_exists('birthday', $participant) && '' !== $participant['birthday']) { /** @phpstan-ignore-line */
                            echo \Sprog\Wildcard::get('d2u_courses_birthdate') .': '. TobiasKrais\D2UCourses\Cart::formatCourseDate($participant['birthday']);
                            $age_seperator = true;
                        } elseif (2 === $ask_age && array_key_exists('age', $participant)) { /** @phpstan-ignore-line */
                            echo \Sprog\Wildcard::get('d2u_courses_age') .': '. TobiasKrais\D2UCourses\Cart::formatCourseDate($participant['age']);
                            $age_seperator = true;
                        }
                        if ($ask_gender) { /** @phpstan-ignore-line */
                            ...
                            echo array_key_exists('gender', $participant) && 'M' === $participant['gender'] ? \Sprog\Wildcard::get('d2u_courses_male') : \Sprog\Wildcard::get('d2u_courses_female');
                        }
                        echo ')';
                    }
                    if ($course->price_salery_level) {
                        ...
                        foreach ($course->price_salery_level_details as $description => $price) {
                            ...
                            if (array_key_exists('price_salery_level_row_number', $participant) && $counter_row_price_salery_level_details === (int) $participant['price_salery_level_row_number']) {
                                echo ', '. \Sprog\Wildcard::get('d2u_courses_fee') .': '. $participant['price'] .' ('. \Sprog\Wildcard::get('d2u_courses_price_salery_level') .': '. $description .')';
                                break;
                            }
                        }
                    }
    ...
    if (rex_addon::get('multinewsletter')->isAvailable() && 'show' === rex_config::get('d2u_courses', 'multinewsletter_subscribe', 'hide')) {
        ...
        if (count($multinewsletter_group) > 0) {
            ...
            if (1 === count($multinewsletter_group)) {
                ...
                echo '<label class="cart_checkbox" for="invoice_form-multinewsletter">'. \Sprog\Wildcard::get('d2u_courses_multinewsletter') .'</label>';
            } else {
                echo \Sprog\Wildcard::get('d2u_courses_multinewsletter') .'<br>';
    ...
    echo '* '. \Sprog\Wildcard::get('d2u_courses_mandatory_fields') .'<br><br>';
    echo '<p class="formsubmit formsubmit">';
    echo '<input type="submit" class="submit save_cart" name="invoice_form[submit]" id="invoice_form-submit" value="'. \Sprog\Wildcard::get('d2u_courses_make_booking') .'">';
    echo '</p>';
?>
	<script>
...
<?php
    ...
    echo '<img src="'.	rex_addon::get('d2u_courses')->getAssetsUrl('cart.png') .'" alt="'. \Sprog\Wildcard::get('d2u_courses_cart') .'">';
    echo '</div>';
    echo '<div class="cart_box_title">'. \Sprog\Wildcard::get('d2u_courses_cart') .'</div>';
    echo '</div>';
    echo '</div>';

    if (0 === count(TobiasKrais\D2UCourses\Cart::getCourseIDs())) {
        echo '<div class="col-12 col-md-9 spacer">';
        echo '<div class="cart_row_title" id="cart_empty">'. \Sprog\Wildcard::get('d2u_courses_cart_empty') .'</div>';
        echo '</div>';
    } else {
...
//					}
            ...
            if ('yes_number' === $course->registration_possible) {
                ...
                echo '<br>';
                echo '<div class="row">';
                echo '<div class="col-12 col-sm-6 col-md-4">'. \Sprog\Wildcard::get('d2u_courses_participant_number') .'</div>'
                ...
                echo '<a href="'. rex_getUrl((int) rex_config::get('d2u_courses', 'article_id_shopping_cart')) .'?delete='. $course_id .'" onclick="return window.confirm(\''. \Sprog\Wildcard::get('d2u_courses_cart_delete_course') .'\');" tabindex="-1">';
                echo '<img src="'. rex_addon::get('d2u_courses')->getAssetsUrl('delete.png') .'" alt="'. \Sprog\Wildcard::get('d2u_courses_cart_delete') .'" class="delete_participant"></a>';
                echo '</div>';
                if ($course->price_salery_level) {
                    echo '<div class="col-12 col-sm-6 col-md-4">'. \Sprog\Wildcard::get('d2u_courses_price_salery_level') .'</div>';
                ...
                foreach (TobiasKrais\D2UCourses\Cart::getCourseParticipants($course_id) as $participant_id => $participant_data) {
                    if (is_array($participant_data)) {
                        ...
                        // First name
                        echo '<div class="col-12 col-sm-6 col-md-4">'. \Sprog\Wildcard::get('d2u_courses_firstname') .'</div>';
                        ...
                        if (1 === TobiasKrais\D2UCourses\Cart::getCourseParticipantsNumber($course_id)) {
                            $ask_delete = ' onclick="return window.confirm(\''. \Sprog\Wildcard::get('d2u_courses_cart_delete_course') .'\');"';
                        }
                        ...
                        // Last name
                        echo '<div class="col-12 col-sm-6 col-md-4">'. \Sprog\Wildcard::get('d2u_courses_lastname') .'</div>';
                        ...
                        // Gender
                        if ($ask_gender) { /** @phpstan-ignore-line */
                            echo '<div class="col-12 col-sm-6 col-md-4">'. \Sprog\Wildcard::get('d2u_courses_gender') .'</div>';
                            ...
                            echo '<option value="M"'. (array_key_exists('gender', $participant_data) && 'M' === $participant_data['gender'] ? ' selected' : '') .'>'. \Sprog\Wildcard::get('d2u_courses_male') .'</option>';
                            echo '<option value="W"'. (array_key_exists('gender', $participant_data) && 'W' === $participant_data['gender'] ? ' selected' : '') .'>'. \Sprog\Wildcard::get('d2u_courses_female') .'</option>';
                            echo '</select></div>';
                        }
                ...
                echo '<div class="row">';
                echo '<div class="col-12">';
                echo '<input type="submit" class="add_participant" name="participant_add['. $course_id .']" value="['. \Sprog\Wildcard::get('d2u_courses_cart_add_participant') .']">';
                echo '</div>';
                echo '</div>';
            }
            echo '</div>';
        ...
        echo '<button type="submit" class="save_cart flex-fill" name="participant_save" value="'. \Sprog\Wildcard::get('d2u_courses_cart_save') .'">'. \Sprog\Wildcard::get('d2u_courses_cart_save') .'</button>';
        echo '</div>';

        echo '<div class="col-12 col-sm-6 spacer d-flex">';
        echo '<input type="submit" class="save_cart flex-fill" name="request_courses" value="'. \Sprog\Wildcard::get('d2u_courses_cart_checkout') .'">';
        echo '</div>';
