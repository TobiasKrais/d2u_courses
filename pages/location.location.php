...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    if (0 === count($uses_courses) && false === $used_in_settings) {
        ...
        if ($used_in_settings) {
            $message .= '<li><a href="index.php?page=d2u_courses/settings/settings">'. rex_i18n::msg('d2u_helper_settings') .'</a></li>';
        }
        $message .= '</ul>';
...
if ('' === $func) {
    ...
    $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###location_id###'] + $csrfToken->getUrlParams());
