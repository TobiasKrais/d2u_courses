...
<p>3.6.3-DEV:</p>
<ul>
        <li>Warenkorb (Module 26-2 und 26-5): Das WYSIWYG-Freitextfeld trägt jetzt einen <code>d2u_translate</code>-Marker. In Verbindung mit d2u_helper (Slice-Übersetzung) erscheint dadurch im Content-Editor ein KI-Übersetzen-Button an diesem Slice.</li>
	<li>Backend: In den Formularen für Kurse und Kategorien wird jetzt direkt an den betroffenen Feldern angezeigt, wofür sie im Frontend/SEO verwendet werden: der Name als Meta-Titel, der Teaser als Meta-Beschreibung und der Hinweis, dass das Bild für die SEO-/Social-Media-Vorschau (<code>og:image</code>) genutzt wird.</li>
	<li>Barrierefreiheit Warenkorb (Module 26-5 und 26-2, WCAG 2.1 AA): Sichtbarer Tastatur-Fokus (<code>:focus-visible</code>) für alle Eingabefelder, Auswahllisten und die Schieberegler-Checkboxen (diese haben durch <code>appearance:none</code> bzw. randlose Felder vorher keinen Fokusrahmen gezeigt). Validierungshinweise (E-Mail-/IBAN-Prüfung, Nicht-SEPA-Warnung) werden per <code>role="alert"</code>/<code>aria-live</code> für Screenreader angesagt. Doppelte HTML-IDs bei mehreren Newsletter-Checkboxen behoben (jetzt eindeutig je Gruppe). Externe Links (AGB, Teilnahmebedingungen, Datenschutz) nutzen jetzt <code>target="_blank" rel="noopener noreferrer"</code> statt <code>target="blank"</code>. Honeypot-Feld ist mit <code>aria-hidden</code> von der assistiven Technik ausgenommen.</li>
	<li>Bugfix Kategorie-/Kurs-URLs: <code>Category::getUrl()</code> und <code>Course::getUrl()</code> lasen die Warenkorb-/Kurs-Artikel-ID aus dem nicht existierenden Konfigschlüssel <code>course_article_id</code> (statt <code>article_id_courses</code>). Dadurch wurde die URL gegen Artikel 0 erzeugt und ohne Domain ausgegeben (z. B. <code>https://sportarten/allround/</code>). Es wird jetzt der korrekte Schlüssel verwendet.</li>
	<li>Warenkorb (Module 26-5 und 26-2): Die Beschriftung des optionalen Freitextfeldes wird jetzt über einen WYSIWYG-Editor gepflegt (statt einzeiligem Text). Im Frontend wird die HTML-Beschriftung ausgegeben; in der Bestätigungsmail wird zur Absicherung nur die Textfassung verwendet.</li>
	<li>Warenkorb (Module 26-5 und 26-2): Die Fragen „Darf mein Kind alleine nach Hause?" und „Unser Kind darf fotografiert werden?" werden jetzt pro Teilnehmer gestellt – nur wenn die Altersabfrage aktiv ist und der jeweilige Teilnehmer minderjährig ist. Die Anzeige wird bereits während der Alterseingabe live umgeschaltet. Die Aktivierung beider Fragen (direkt unter der Altersabfrage eingeblendet) sowie das optionale Freitextfeld werden jetzt direkt im Modul konfiguriert statt in den Addon-Einstellungen. Die Werte werden pro Teilnehmer gespeichert, in die Bestätigungsmail und (bei aktivem <code>customer_bookings</code>) je Buchung gespeichert; neue Spalte <code>photo_permission</code>. Modul-Revisionen erhöht.</li>
	<li>Bugfix Warenkorb (Modul 26-5): Das Label des Geburtsdatum-/Alter-Feldes im Teilnehmerformular wurde wegen undefinierter Variablen (<code>$tag_open</code>/<code>$tag_close</code>) leer ausgegeben. Es wird jetzt korrekt über <code>Sprog\Wildcard::get()</code> ausgegeben.</li>
	<li>Warenkorb (Modul 26-5, Dark Mode): Die Eingabefelder zeigten im Dark Mode weiße Schrift auf hellem Grund (unlesbar) und der Hintergrund der Checkbox-Zeilen (<code>cart_checkbox</code>) blieb hell. Text- und Hintergrundfarben nutzen jetzt <code>light-dark()</code> und passen sich dem Theme an. Zusätzlich sind die Schieberegler-Checkboxen gegen ein projektweites <code>input[type="checkbox"]{width:auto}</code> abgesichert (höhere Selektor-Spezifität), damit die Darstellung als Schieberegler erhalten bleibt.</li>
	<li>Bugfix: Beim Speichern eines Kurses trat eine <code>ErrorException</code> („Undefined array key …") für Felder auf, die je nach aktivem Plugin bzw. Importtyp nicht angezeigt werden: Teilnehmerzahl und Warteliste (bei aktivem <code>customer_bookings</code>) sowie die Gehaltsstufen-Details (bei <code>kufer_sync</code>/KuferSQL-Import). Diese Werte werden jetzt nur noch übernommen, wenn die Felder im Formular vorhanden sind; ansonsten bleibt der gespeicherte Wert erhalten.</li>	<li>Warenkorb (Module 26-2 und 26-5): Checkboxen werden jetzt als Schieberegler dargestellt. Neues optionales Freitextfeld, dessen Beschriftung in den Einstellungen frei definiert werden kann (leer = ausgeblendet). Bei Kinderkursen (minderjährige Teilnehmer im Warenkorb oder aktivierte Einstellung) werden die Fragen „Darf mein Kind alleine nach Hause?“ und „Unser Kind darf fotografiert werden?“ angezeigt. Freitext und Foto-Erlaubnis werden in die Buchungs-Bestätigungsmail aufgenommen. Modul-Revisionen erhöht.</li>
...
<ul>
	...
	<li>Security: In <code>modules/26/4/output.php</code> werden jetzt auch die Listen-Titel (Zielgruppen-, Termin-Kategorie-, Standort-, Standortkategorie- und Kategorie-Namen) sowie Kurs-Titel und Uhrzeit in der Listenausgabe mit <code>rex_escape()</code> ausgegeben. Rich-Text-Felder (Beschreibung/Teaser) bleiben bewusst unverändert. In <code>boot.php</code> werden die Datensatz-Namen in den Lösch-Warnungen (article-/media-is-in-use) mit <code>rex_escape()</code> gehärtet.</li>
	<li>Security: Auch in den weiteren Beispielmodulen werden Backend-Eingaben gegen XSS gehärtet: In <code>modules/26/2</code> und <code>26/5</code> werden Kurs-Name und Kurs-Nummer im Warenkorb sowie in den Link-Texten mit <code>rex_escape()</code> ausgegeben; in den Input-Formularen von <code>26/1</code>, <code>26/2</code>, <code>26/4</code> und <code>26/5</code> werden Kategorie-Namen in den <code>&lt;option&gt;</code>-Labels mit <code>rex_escape()</code> ausgegeben. Rich-Text-Felder (Beschreibung/Teaser) bleiben bewusst unverändert.</li>
	<li>Intern: In den Modulen <code>26/3</code> und <code>26/6</code> wird der Kategorie-Zugriff für die Box-Farben jetzt über einen <code>instanceof</code>-Guard abgesichert (Typsicherheit, keine Verhaltensänderung).</li>
