...
$sql->setQuery('CREATE OR REPLACE VIEW '. rex::getTablePrefix() .'d2u_courses_url_schedule_categories AS
	...
	WHERE courses.online_status = "online"
		...
		AND courses.updatedate = (
			...
			WHERE schedules.schedule_category_id = schedules_max.schedule_category_id AND courses_max.online_status = "online" AND (date_start = "" OR date_start > CURDATE())
		)
	...
	WHERE parents.schedule_category_id > 0
		...
		AND courses.updatedate = (
			...
			WHERE schedules.parent_schedule_category_id = schedules_max.schedule_category_id AND courses_max.online_status = "online" AND (date_start = "" OR date_start > CURDATE())
		)
	GROUP BY schedule_category_id, name, seo_title, picture, updatedate, parent_schedule_category_id;');
