...
function rex_d2u_courses_locations_article_is_in_use(rex_extension_point $ep)
{
    ...
    if ($addon->hasConfig('article_id_locations') && (int) $addon->getConfig('article_id_locations') === $article_id) {
...
function rex_d2u_courses_locations_media_is_in_use(rex_extension_point $ep)
{
    ...
    for ($i = 0; $i < $sql_categories->getRows(); ++$i) {
        $message = rex_i18n::msg('d2u_courses_rights_all') .' - '. rex_i18n::msg('d2u_courses_location_categories') .': <a href="javascript:openPage(\'index.php?page=d2u_courses/location/category&func=edit&entry_id='. $sql_categories->getValue('location_category_id') .'\')">'.
